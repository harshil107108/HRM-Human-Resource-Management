...
export default function PhoneField({ field, form }) {
    ...
    const sanitizePhone = useCallback((rawValue = "") => {
        return String(rawValue).replace(/\D/g, "").slice(0, 10);
    }, []);

    const handleChange = useCallback(
        (event) => {
            form.methods.setValue(id, sanitizePhone(event.target.value));
        },
        [form, id, sanitizePhone],
    );
