...
const Designation = () => {
  ...
  const {
    ...
    payrollSchema,
    payrollToggleConfig,
  } = useDesignationConfig({ EmployeeData });
  ...
  const handleSave = async () => {
    const result = await formmethod.methods.handleFormSave(
      ...
      },
      {
        onSuccess: async () => {
          successAlert({
            title: DesignationId ? "Designation Updated" : "Designation Added",
            text: DesignationId
  ...
  const getDataById = async () => {
    ...
    if (res?.success) {
      ...
      setToggleData({
        professionalTaxApplicable: data.professionalTaxApplicable ?? false,
  ...
  return (
    <>
      <div className="hp-company-page">
        <HpHeader title="Designation" className="hp-company-page__header" />
